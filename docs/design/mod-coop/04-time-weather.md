...
| 5.10 | **Room layers at night** | Layers resolve at stage/room load from `dComIfG_get_timelayer()`. A client that loads a stage with a stale time could pick the wrong layer until next room load. Mitigation: client sets the replicated time as early as possible (post-hook on `dKy_Create` → `dComIfGs_setTime`) before room layer resolution — **verify ordering during implementation**; worst case is one wrong layer until the next room change (acceptable v1). |
| 5.11 | **Fishing pond 2x time** (F_SP127/R_SP127) | Covered by `rate=3`; the vanilla double-advance rule (only between 300-60 and 150-195) is replicated by the client clock only if exactness matters — otherwise `rate=3` (uniform 2x) is visually indistinguishable (lighting is smooth) and self-corrects at 1 Hz. |
