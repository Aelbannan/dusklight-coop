...
| 5.10 | **Room layers at night** | Layers resolve at stage/room load from `dComIfG_get_timelayer()`. A client that loads a stage with a stale time could pick the wrong layer until next room load. Mitigation (verified in M4 join-warp): the client seeds the host's time from `WorldInit` BEFORE requesting the stage change, and passes the host's layer explicitly (`dComIfGp_setNextStage(..., hostLayer, ...)`), so the time-derived resolution matches the host's layer even when the engine re-resolves. |
